...
def verify_hashes(password,hashed_text):
    if generate_hashes(password)==hashed_text:
        return hashed_text
    return false
log_home=st.checkbox('Login To Start')
def main():
    #"login page"
    st.title("Login")
    menu=["Login","SignUp"]
...
if log_home:
    ...
    if pages=="Home":
        ...
        intro_markdown = read_markdown_file("intro2.md")
        ...
        col2 = st.selectbox('Which feature on y?', ['age_years','marital_status','degree_2','country_birth','pregnancy','household_income','US_citizen','gender'])
        if col1=="Gender":
            if col2=="age_years":
        ...
        if col1=="Education":
            ...
            if col2=="household_income":
                ...
                st.pyplot()
            if col2=="pregnancy":
                demo1.groupby(['Education','pregnancy']).size().unstack().plot(kind='bar',stacked=True,color=('darkred','silver','darkgrey'))
                st.pyplot()
        ...
        if radio=="Alcohol Consumption":
            ...
            fig = px.scatter(alc2, x="index", y="ALQ130")
            ...
            if cig=="Adults":
                ...
                ad1=ad.value_counts().reset_index()
                st.table(ad1)
                fig = px.bar(ad1, x='index', y='SMQ020')
                st.plotly_chart(fig)

            elif cig=="Teenagers":
                teen=df_ques['SMQ621'].dropna()
                teen1=teen.value_counts().reset_index()
                st.table(teen1)
                fig = px.bar(teen1, x='index', y='SMQ621')
                st.plotly_chart(fig)

    elif pages=='Nutrition and Health':
        st.write("Nutrition and Health")
        exam=pd.read_csv('exam.csv')
        #Obesity
        exam['BMI'] = exam['BMI'].fillna(exam['weight_kg'] / (exam['height_cm'] / 100)**2).dropna()

        fig = px.histogram(ed, x="Education", nbins=30)
    ...
    if pages2=="Diabetes Prediction":
        ...
        if machinelearn:
            alg = ['Support Vector Machine','KNN','Random Forest']
